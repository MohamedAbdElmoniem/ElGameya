...
export default class NormalCycleView extends Component {
  ...
  renrederMembers() {
    ...
    axios({
      method: "POST",
      url: "http://www.elgameya.net/api/gamieya/GetCycleMembersForNonAdmin",
      data: JSON.stringify({
      ...
      .then(resp => {
        ...
        let numberOfMembers = this.props.navigation.state.params.cycleid
          .numbeR_OF_MEMBERS;
        ...
        for (var x = 0; x < numberOfMembers; x++) {
          ...
          if (findMemberByMonth) {
            ...
            let shareOptions = {
              ...
              message:
                "join gameya : " +
                this.props.navigation.state.params.cycleid.cyclE_NAME +
                "from this link : https://elgameya/gameya/" +
                this.props.navigation.state.params.cycleid.cyclE_NAME +
                "/" +
                parseInt(moment(months[x]).format("M")),
              url:
                "https://elgameya.net/gameya/" +
                this.props.navigation.state.params.cycleid.cyclE_NAME +
                "/" +
                parseInt(moment(months[x]).format("M")),
            ...
            renderedMembers.push(
              ...
              >
                <Text style={{ color: "#262261", fontSize: 8 }}>
                  {months[x]}
                </Text>
                <Image
        ...
        this.setState(
          { renderedMembers: renderedMembers, noMembers: false },
          () => {
      ...
      .then(resp => {
        ...
        this.setState({ profileResponse: response }, () => {
          for (let x = 0; x < response.user.useR_FOLLOWERS.length; x++) {
            if (
              response.user.useR_FOLLOWERS[x].follower_id ===
                this.currentUserId &&
              response.user.id === response.user.useR_FOLLOWERS[x].user_id
            ) {
              this.flag = true;
            }
          }
  ...
  handleSaveReminders = () => {
    if (this.state.remindersFlag === true) {
      ...
      var dateStart = moment(
        moment(this.props.navigation.state.params.cycleid.startDate).format(
          "YYYY-MM-DD"
        )
      );
      var dateEnd = moment(
        moment(this.props.navigation.state.params.cycleid.endDate).format(
          "YYYY-MM-DD"
        )
      );
      var timeValues = [];
        ...
        deferedListOfEvents.push(
          RNCalendarEvents.saveEvent(
              ...
              this.props.navigation.state.params.cycleid.cyclE_NAME,
            {
              ...
              notes:
                "pay : " +
                this.props.navigation.state.params.cycleid.totaL_AMOUNT,
              startDate:
                dateStart.format("YYYY-MM-DD").toString() + "T14:00:00.000Z",
              endDate:
                dateStart.format("YYYY-MM-DD").toString() + "T14:00:00.000Z"
            }
          )
      ...
      axios.all(deferedListOfEvents).then(results => {
        results.forEach((response, i) => {
          this.state.arrayOfReminders.push({
            ...
            date:
              timeValues[i].format("YYYY-MM-DD").toString() + "T14:00:00.000Z"
          });
        ...
        .then(resp => {
          ...
          if (finalData.reminders) {
            for (let x = 0; x < finalData.reminders.length; x++) {
              defferedRemoveEvents.push(
                RNCalendarEvents.removeEvent(
                  finalData.reminders[x].calendar_reminder_id.toString()
                )
              );
            }
  ...
  renderPopupMenu() {
    return (
      <Menu
        onSelect={value => {
          this.handlePopupMenuClick(value);
        }}
      >
        <MenuTrigger>
  ...
  handlePopupMenuClick(value) {
    ...
    if (value === "Group Message") {
      ...
      navigate("GroupChat", {
        cycle: this.props.navigation.state.params.cycleid,
        userid: this.props.navigation.state.params.userid,
        page:"NormalCycleView"
      });
    }
  ...
  render() {
    ...
    let shareOptions = {
      title: "ElGameya app",
      message:
        "i would like to invite you to join cycle : " +
        cycleInfo.cyclE_NAME +
        " and pay : " +
        cycleInfo.totaL_AMOUNT +
    ...
    return (
      <Container>
        ...
        <Modal
          ...
          style={[styles.modal, styles.modalProfile]}
          position={"center"}
          ref={"ModalProfile"}
          swipeToClose={false}
        ...
        >
          <Container>
            <Content>
              {this.state.showData
                ? this.renderProfileData(this.state.profileResponse)
                : null}
            </Content>
          </Container>
        </Modal>
        ...
        <Modal
          backButtonClose={true}
          style={[styles.modal, styles.ModalEditList]}
          position={"center"}
          ref={"ModalEditList"}
          swipeToClose={false}
          isDisabled={this.state.isDisabled}
        >
          <View style={{ height: 600, flex: 1, flexDirection: "column" }}>
            <Header
              ref="myheader"
              style={{ backgroundColor: "#9E1F64", marginBottom: 10 }}
            >
              <Left />
              <Body>
            ...
            <SortableList
                  ...
                  >
                    ...
                    <View style={{ flex: 0.5 }}>
                      <Text style={{ color: "#262261" }}>
                        Name : {data.data.username} month : {data.data.month}
                      </Text>
                    </View>
                    <View style={{ flex: 0.2 }} />
                    <View style={{ flex: 0.2 }}>
              ...
              onChangeOrder={this.changeMembersListOrder}
            />
          </View>
        </Modal>
        ...
        <Modal
          backButtonClose={true}
          style={[styles.modal, styles.ModalDeleteCycle]}
          position={"center"}
            ...
            <Content>
              <Grid>
                ...
                <Row>
                  <Col style={{ width: "40%" }}>
                    <Button
                      style={{ borderRadius: 10, backgroundColor: "#9E1F64" }}
                      onPress={() => {
              ...
              <View style={{ flex: 1, flexDirection: "row" }}>
                ...
                <View style={{ flex: 0.3 }}>
                  <Switch
                    onValueChange={val => {
                      this.setState({
                        remindersFlag: !this.state.remindersFlag
                      });
                    }}
                    value={this.state.remindersFlag}
                  />
                </View>
              </View>
              <View
              ...
              <Right>
                <Grid>
                  <Row style={{ marginTop: 15 }}>
                    ...
                    <Col style={{ width: "50%" }}>{this.renderPopupMenu()}</Col>
                    <Col style={{ width: "20%" }}>
                      <Icon
                        onPress={() => {
            ...
            <View style={{ flex: 1, flexDirection: "row" }}>
              <View style={{ marginLeft: 20, marginTop: 12, marginBottom: 12 }}>
                <Thumbnail
                  source={{
                    uri:
                      "https://d30y9cdsu7xlg0.cloudfront.net/png/4195-200.png"
                  }}
                />
              </View>
              ...
              <View style={{ marginLeft: 20, marginTop: 23 }}>
                ...
                <Text style={{ fontSize: 16 }}>
                  number of memebers :{" "}
                  {this.props.navigation.state.params.cycleid.numbeR_OF_MEMBERS}
                </Text>
              </View>
            </View>
